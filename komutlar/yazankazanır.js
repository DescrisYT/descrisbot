...
exports.run = async (client, msg, args) => {
  ...
  if (opponent.bot) return msg.reply('Botlar ile oynayamazsın!');
		...
		try {
			await msg.channel.send(`<a:hypesquad1:821515921339318322> ${opponent} <a:hypesquad1:821515921339318322> bu meydan okumayı kabul ediyor musun? (\`evet\` veya \`hayır\` olarak cevap veriniz.) <a:Fire:821514165436284949> `);
			...
			return msg.channel.send(`Hızlıymışsın! Tebrikler ${winner.first().author} Kazandın! <a:rengarenkigne:821514216711258142> `);
		} catch (err) {
			this.fighting.delete(msg.channel.id);
			throw err;
		}
