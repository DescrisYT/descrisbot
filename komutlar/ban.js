...
let sebeb = args.slice(1).join(' ') || 'Belirtilmemiş'
 ...
 if (!sebeb) {
         try {
          kisi.send(`<a:rengarenkigne:821514216711258142> ${kisi} **${guild}** adlı sunucudan banlandınız. sebeb: \`${sebeb}\``)
 ...
 try {
   kisi.send(`<a:rengarenkigne:821514216711258142> ${kisi} **${guild}** adlı sunucudan banlandınız. \nNedeni: **${sebeb}**`)
   message.channel.send(`**${kisi} Yasaklandı. \nNedeni:** \`${sebeb}\``)
   guild.members.ban(kisi, { reason: sebeb/*, days: gun*/});
 } catch (error) {
   message.reply("Bir hata oluşdu rolüme bakmaya ne dersin")
